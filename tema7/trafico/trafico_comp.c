/*
 ...
 - Carta AD: Calles aleatorias y giros en cruces.
 - Carta BE: Semáforos y entradas de coches aleatorias.
 - Carta C: Múltiples coches y control de colisiones.
...
#define NUM_CALLES_H 2
#define NUM_CALLES_V 2
...
void nuevo_coche(Coche *coche, Calles *calles); // Versión AD adaptada
...
int main() {
  ...
  for (int turno = 0; turno < TURNOS; turno++) {
    ...
    mover_coches(mapa, coches, semaforos, num_semaforos);
...
void generar_calles(Calles *calles) {
  // Genera 2 calles horizontales y 2 verticales aleatorias
...
char tipo_carretera(int i, int j, Calles *calles) {
  // Determina si una celda es calle, cruce o vacío
  bool es_h = (i == calles->filas_h[0] || i == calles->filas_h[1]);
  bool es_v = (j == calles->cols_v[0] || j == calles->cols_v[1]);
...
void actualizar_semaforos(Semaforo semaforos[], int num_semaforos, int turno) {
  for (int i = 0; i < num_semaforos; i++) {
    // Cambiar el estado según el ciclo
    // Se puede añadir un desfase basado en i si se quiere variedad
    semaforos[i].estado = (turno / CICLO_SEMAFORO) % 2 == 0 ? VERDE_H : VERDE_V;
  }
}
